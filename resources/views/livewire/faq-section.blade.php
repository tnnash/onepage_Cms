                ...
                <div>
                    @foreach($faqs as $faq)
                        ...
                        <div class="mb-6 bg-white p-6 rounded-lg">
                            <button class="w-full text-left text-lg font-semibold text-gray-800 flex justify-between items-center hover:text-blue-600 focus:outline-none transition-colors duration-200" onclick="toggleFAQ({{ $faq->id }})">
                                <span>{{ $faq->question }}</span>
                                <svg id="faq-arrow-{{ $faq->id }}" class="w-6 h-6 text-gray-600 transform transition-transform duration-200" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                    <path d="M6 9l6 6 6-6"></path>
                                </svg>
                            </button>
                            <div id="faq-{{ $faq->id }}" class="text-gray-600 hidden mt-2 transition-all ease-in-out duration-300">
                                <p>{{ $faq->answer }}</p>
                            </div>
                        </div>
                    @endforeach
