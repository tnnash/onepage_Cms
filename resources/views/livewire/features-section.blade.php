<section id="features" class="py-16 bg-white flex items-center justify-center">
    <div class="container mx-auto text-center">
        <h2 class="text-3xl font-bold mb-8">{{ $sectionTitle }}</h2>
        <div class="w-16 h-1 bg-blue-600 mx-auto mb-4"></div>
        <!-- Filter Buttons -->
        <div class="flex justify-center space-x-4 mb-8">
            @foreach ($features as $index => $feature)
                <button 
                    class="px-4 py-2 bg-gray-200 rounded-md shadow hover:bg-blue-500 hover:text-white filter-btn {{ $index === 0 ? 'bg-blue-500 text-white' : '' }}" 
                    data-filter="tab{{ $feature->id }}">
                    {{ $feature->title }}
                </button>
            @endforeach
        </div>
        ...
        <div id="feature-content">
            @foreach ($features as $index => $feature)
                <div class="feature-tab tab{{ $feature->id }} {{ $index === 0 ? '' : 'hidden' }}">
                    <div class="flex flex-col md:flex-row items-center justify-center mb-8">
                        <div class="md:w-1/2">
                            <!-- Enlarge and Align Left -->
                            <h3 class="text-4xl font-bold text-left mb-4">{{ $feature->title }}</h3>
                        ...
                        <div class="md:w-1/2 flex justify-center">
                            <img src="{{ $feature->image_url }}" alt="{{ $feature->title }}" class="rounded-lg max-w-md w-full h-auto">
                        </div>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        @foreach ($feature->tiles as $tile)
                            <div class="p-6 tile bg-gray-100 rounded-lg shadow-md">
                                <h4 class="text-lg font-semibold">{{ $tile->name }}</h4>
