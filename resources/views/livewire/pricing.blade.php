        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            @foreach($pricingPlans as $index => $plan)
                <div class="bg-white p-8 rounded-lg shadow-lg 
                    @if($index == 1) bg-blue-600 text-white @endif
                    transition-colors duration-300 ease-in-out">
                    <h3 class="text-2xl font-semibold mb-4">{{ $plan->title }}</h3>
                    <p class="text-4xl font-bold text-black mb-4 inline">
                        @if($index == 1) {{ $plan->price }} @else <span class="text-blue-600">{{ $plan->price }}</span> @endif
                    </p>
                    ...
                    <a href="{{ $plan->button_link }}" class="
                        @if($index == 1) bg-white text-blue-600 @else bg-blue-600 text-white @endif
